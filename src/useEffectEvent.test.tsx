...
test('useEffectEvent creates functions with a stable reference (the same function on every render)', () => {
  // The native hook returns a new closure on every render instead. This ponyfill guarantees a
  // stable identity so that listing the function in a dependency array is always harmless —
  // linters ported from the React Compiler (e.g. oxlint's
  // `react/exhaustive-effect-dependencies`) only exempt the native hook and push callers into
...
describe('freshness inside wrapper components (facebook/react#34818)', () => {
  // On React 19.2 the native useEffectEvent keeps reading first-render values forever when
  // the component is wrapped in React.memo or React.forwardRef. The ponyfill must not share
  // that bug — see test/react-19.2 for the side-by-side comparison against the native hook.
